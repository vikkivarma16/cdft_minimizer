...
def potential_r_k_space():
    ...
    # Bessel Fourier Transform (Hankel Transform) for radially symmetric potentials
    ...
    '''def hard_core_potential(r, epsilon, sigma):
        ...
        for radius in r:
            value.append(0.0)
            if (float(radius) >= sigma-EPSILON and float(radius) <= sigma+EPSILON):
                value.append(10.0)
    ...
    def hard_core_potential(r, epsilon, sigma):
        """Smooth Hard-core repulsive potential for numerical stability.
        r (float or array): Distance(s) at which to calculate the potential.
        sigma (float): Particle diameter (radius of hard-core).
        large_value (float): Large value for potential just below sigma.
        smoothing_width (float): Smoothing range around sigma.

        Returns:
        float or array: Smoothed potential values at distance(s) r.
        """
    ...
    def wca_potential_1(r, epsilon, sigma):
        """Calculate the Weeks-Chandler-Andersen (WCA) potential.

        Parameters:
        r (float or array): Distance(s) between particles.
        epsilon (float): Depth of the potential well.
        sigma (float): Particle diameter.

        Returns:
        float or array: WCA potential values at distance(s) r.
        """
        # Define the cutoff for the WCA potential (at r = 2^(1/6) * sigma)
        # Calculate the Lennard-Jones part of the potential for r < r_cutoff
    ...
    def calculate_interactions(json_file):
        ...
        for i, pair_type in enumerate(all_k_space["tertiary"].keys()):
            axes[1].plot(all_k_space["tertiary"][pair_type], all_V_k["tertiary"][pair_type], 
                          label=f'{pair_type} {"tertiary"} ({interaction_types["secondary"][pair_type].capitalize()})', 
                          linewidth=2.5, 
                          linestyle=line_styles[i % len(line_styles)], 
                          color=colors[i % len(colors)])  # Cycle through line styles and colors
    ...
    json_file_path = 'input_interactions_properties.json'  # Make sure this file exists and follows the correct format
